...
import ca.sfu.hiaccel.MemoryManager.{GlobalAllocator, JavaMemoryPoolServer}
...
class NativeORCReader(val fileName: String, val inputSchema: Schema,
val outputSchema: Schema, val batchSize: Int) extends Iterator[VectorSchemaRoot] {
  ...
  private val ptr: Long = {
    ...
    initNativeORCReader(memoryPool, fileName, inputSchemaBytes, outputSchemaBytes, batchSize)
  }
  ...
  @native def initNativeORCReader(jMemoryPool: JavaMemoryPoolServer, fileName: String, inputSchemaBytes: Array[Byte], outputSchemaBytes: Array[Byte], numRows: Int): Long
