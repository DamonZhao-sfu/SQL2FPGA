...
import ca.sfu.hiaccel.NativeORCReader
import ca.sfu.hiaccel.MemoryManager.GlobalAllocator

import scala.collection.JavaConverters._

@RunWith(classOf[JUnitRunner])
class NativeORCReaderSuite extends AnyFunSuite {
  ...
  test("read int, long and string from parquet file") {
    val reader = new NativeORCReader(fileName, inputSchema, schema(intField, longField, stringField), 50000)
  ...
  test("that reading can be limited to subset of fields") {
    val reader = new NativeORCReader(fileName, inputSchema, schema(longField), 50000)
  ...
  test("that the whole iterator can be processed") {
    ...
    val reader = new NativeORCReader(fileName, inputSchema, schema(intField), 60000)
  ...
  ignore("read 500 million values"){
    ...
    //read only the int-field
    val reader = new NativeORCReader("../data/500-million-int-triples-snappy.parquet", schema, schema, 445678)
