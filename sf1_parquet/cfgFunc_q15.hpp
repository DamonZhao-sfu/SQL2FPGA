...
void get_cfg_dat_Aggregate_TD_5151_gqe_aggr(ap_uint<32>* buf) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: Aggregate
    // Operation: ListBuffer(l_suppkey#66, sum((l_extendedprice#117 * (100 - l_discount#134))) AS total_revenue#1509L)
    ...
    // Output Table: ListBuffer(total_revenue#1509L)
    // Node Depth: 5
    ...
    shuffle4_cfg(7, 0) = 3; // total_revenue#1509L
    shuffle4_cfg(15, 8) = -1;
...
void get_cfg_dat_Aggregate_TD_4759_gqe_aggr(ap_uint<32>* buf) {
    // StringRowIDSubstitution: false StringRowIDBackSubstitution: false
    // Supported operation: Aggregate
    // Operation: ListBuffer(l_suppkey#66, sum((l_extendedprice#117 * (100 - l_discount#134))) AS sum((l_extendedprice * (100 - l_discount)))#1507L)
        // Binded Operation: Project -> operations: ListBuffer(cast(l_suppkey#2L as int) AS l_suppkey#66, cast(CheckOverflow((promote_precision(l_extendedprice#5) * 100.00), DecimalType(16,2), true) as int) AS l_extendedprice#117, cast(CheckOverflow((promote_precision(l_discount#6) * 100.00), DecimalType(16,2), true) as int) AS l_discount#134)
    // Input Table: ListBuffer(l_suppkey#2L, l_extendedprice#5, l_discount#6)
    // Output Table: ListBuffer(l_suppkey#66, sum((l_extendedprice * (100 - l_discount)))#1507L)
    // Node Depth: 4
    ap_uint<32>* config = buf;
    memset(config, 0, sizeof(ap_uint<32>) * 83);
    ...
    ap_int<64> shuffle4_cfg;
    shuffle4_cfg(7, 0) = 3; // sum((l_extendedprice * (100 - l_discount)))#1507L
    shuffle4_cfg(15, 8) = -1;
    shuffle4_cfg(23, 16) = -1;
    shuffle4_cfg(31, 24) = -1;
    shuffle4_cfg(39, 32) = -1;
...
void get_cfg_dat_JOIN_INNER_TD_1963_gqe_join(ap_uint<512>* hbuf) {
    // StringRowIDSubstitution: true StringRowIDBackSubstitution: false
    // Supported operation: JOIN_INNER
    // Operation: ListBuffer((s_suppkey#498 = supplier_no#1508))
    ...
    // Right Table: ListBuffer(supplier_no#1508, total_revenue#1509L)
    // Output Table: ListBuffer(s_suppkey#498, s_name#485, s_address#486, s_phone#488, total_revenue#1509L)
    // Node Depth: 1
    ap_uint<512>* b = hbuf;
