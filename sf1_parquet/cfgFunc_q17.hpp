...
void get_cfg_dat_Project_TD_4323_gqe_join(ap_uint<512>* hbuf) {
    ...
    shuffle3_cfg(7, 0) = 1; // l_quantity
    shuffle3_cfg(15, 8) = 8; // l_partkey
    shuffle3_cfg(23, 16) = 8; // l_partkey
    ...
    shuffle4_cfg(7, 0) = 2; // l_partkey
    shuffle4_cfg(15, 8) = 8; // l_quantity
...
void get_cfg_dat_JOIN_INNER_TD_2929_gqe_join(ap_uint<512>* hbuf) {
    ...
    // Operation: ListBuffer((p_partkey#420 = l_partkey#49))
        // Binded Operation: Project -> operations: ListBuffer(cast(l_partkey#1L as int) AS l_partkey#49, cast(CheckOverflow((promote_precision(l_quantity#4) * 100.00), DecimalType(16,2), true) as int) AS l_quantity#100, cast(CheckOverflow((promote_precision(l_extendedprice#5) * 100.00), DecimalType(16,2), true) as int) AS l_extendedprice#117)
    ...
    // Right Table: ListBuffer(l_partkey#1L, l_quantity#4, l_extendedprice#5)
    // Output Table: ListBuffer(l_quantity#100, l_extendedprice#117, p_partkey#420)
    // Node Depth: 2
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;
