...
void get_cfg_dat_Project_TD_2539_gqe_join(ap_uint<512>* hbuf) {
    ...
    // Left Table: ListBuffer(p_partkey#402L, p_size#406, p_container#407, p_brand#410)
    // Right Table: ListBuffer()
    // Output Table: ListBuffer(p_partkey#420, p_size#406, p_container#407, p_brand#410)
    // Node Depth: 2
    ap_uint<512>* b = hbuf;
    ...
    shuffle3_cfg(15, 8) = 1; // p_size
    shuffle3_cfg(23, 16) = 2; // p_container
    shuffle3_cfg(31, 24) = 3; // p_brand
    shuffle3_cfg(39, 32) = 8; // p_partkey
    ...
    shuffle4_cfg(7, 0) = 4; // p_partkey
    shuffle4_cfg(15, 8) = 1; // p_size
    shuffle4_cfg(23, 16) = 2; // p_container
...
void get_cfg_dat_Project_TD_2674_gqe_join(ap_uint<512>* hbuf) {
    ...
    // Left Table: ListBuffer(l_partkey#1L, l_quantity#4, l_extendedprice#5, l_discount#6)
    // Right Table: ListBuffer()
    // Output Table: ListBuffer(l_partkey#49, l_quantity#100, l_extendedprice#117, l_discount#134)
    // Node Depth: 2
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ap_uint<512> t = 0;
    ...
    ap_uint<289> op_eval_0 = 0; // l_partkey#1L
    xf::database::dynamicALUOPCompiler<uint32_t, uint32_t, uint32_t, uint32_t>("strm1", 0, 0, 0, 0, op_eval_0);
    b[1] = op_eval_0;
