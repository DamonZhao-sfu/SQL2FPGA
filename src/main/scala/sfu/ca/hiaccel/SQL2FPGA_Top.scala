...
object SQL2FPGA_Top {
  ...
  val INPUT_DIR_TPCDS = "/localhdd/hza215/spark_benchmark/tpcds/orc_1"
  ...
  def outputDF(
      query: SQL2FPGA_Query,
      df: DataFrame,
      outputDir: String,
      elapsed_time: Float): Unit = {
  ...
  def executeTPCHQueries(
      sc: SparkSession,
      schemaProvider: TpchSchemaProvider,
      qConfig: SQL2FPGA_QConfig,
      output_dir: String): ListBuffer[(String, Float)] = {
    ...
    for (queryNo <- queryNumList) {
      ...
      val query = Class
        .forName(f"sfu.ca.hiaccel.TPCH.TPCH_Q$queryNo%02d")
        .newInstance()
        .asInstanceOf[TPCH_Queries]
      ...
      for (rep_idx <- 0 until qConfig.num_spark_execution) {
        ...
        codegen.writeSparkSQLEvaluationResults(
          queryNo,
  ...
  def executeTPCDSQueries(
      sc: SparkSession,
      schemaProvider: TpcdsSchemaProvider,
      qConfig: SQL2FPGA_QConfig,
      output_dir: String): ListBuffer[(String, Float)] = {
    var codegen = new SQL2FPGA_Codegen
    val results = new ListBuffer[(String, Float)]
    ...
    for (queryNo <- queryNumList) {
      println("Start of SparkSQL Execution")
      val query = Class
        .forName(f"sfu.ca.hiaccel.TPCDS.TPCDS_Q$queryNo%02d")
        .newInstance()
        .asInstanceOf[TPCDS_Queries]
  ...
  def main(args: Array[String]): Unit = {
    ...
    qConfig.tpcds_queryNum_list = ListBuffer(
      72
      //4, 9, 10, 11, 14, 23, 24, 28, 72, 75 , 77, 78, 80, 85, 88, 93, 94, 95
    ...
    val bw = new BufferedWriter(new FileWriter(outFile, true))
    output.foreach { case (key, value) => bw.write(f"$key%s\t$value%1.8f\n") }
    bw.close()
  }
}
